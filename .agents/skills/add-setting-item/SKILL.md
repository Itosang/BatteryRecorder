...
description: 为 BatteryRecorder 仓库新增单个设置项的固定流程。用于需要修改设置链路、区分 AppSettings/StatisticsSettings/ServerSettings，尤其是新增 ServerSettings 项并检查 ServerSettingsCodec、SharedSettings、ConfigProvider、ConfigUtil、IService.aidl、Server.updateConfig 等同步节点时。
---
...
7. `ServerSettingsCodec` 是 `ServerSettings` 字段映射的唯一入口；`SharedSettings` 与 `ConfigUtil` 只负责来源适配，不再各自维护一份字段清单。
