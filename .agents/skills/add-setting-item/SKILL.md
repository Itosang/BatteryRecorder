...
1. 先判断设置项归属：`AppSettings`、`StatisticsSettings`、`ServerSettings`。不要把 `ServerConfigDto` 当成设置真值，也不要再按旧 `coerceConfigValue` 心智模型找入口。
...
6. 不要恢复零散 `SharedPreferences` 直读直写，也不要把默认值/裁剪逻辑散落到 UI 或 IPC 层；Server 配置合法化统一看 `SharedSettings.normalizeServerSettings(...)` / `SharedSettings.serverSettingsFromStoredValues(...)`。
7. `ConfigUtil` 当前职责是“来源适配后回到 `ServerSettings`”，不是继续维护独立的 DTO 合法化包装层；如果看到仅为 DTO 往返存在的薄包装方法，不要继续强化这类路径。
